...
def hmm_arbitrary(ic_slice, dimension, dims, n_clusters):
#
	populations, ranges, min_pop, tot_pop = md_populations(ic_slice, dimension)
	
	hmm = GHMM(n_states=n_clusters, reversible_type='transpose', thresh=1e-4, init_algo='GMM', timing=True)
	hmm.fit(ic_slice)
	hmm_popl = hmm_populations(hmm, dimension, ranges, min_pop)
	BIC = (1.0 + 2.0 * float(dims)) * float(n_clusters)**2 * math.log(tot_pop) - 2.0 * hmm.fit_logprob_[0]
